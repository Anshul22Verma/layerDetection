...
def pre_train_model(
    ...
    print("="*10 + " Pre-training the Model using SimSiam Learning " + "="*10)
...
def train_classifier_w_pretraining(
    ...
    print("="*10 + " Fine-tuning model Pre-trained using SimSiam Learning " + "="*10)
    ...
    model = torch.load(os.path.join(model_dir, f"simsiam_ft_{pre_trained_model.name}.pth"), weights_only=False)
    return model
